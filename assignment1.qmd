...
The first thing that stood out is how strongly Auckland CBD still behaves like a workweek city.

Figure 1. Mean hourly pedestrian counts across Auckland CBD. Weekday activity stays above weekend activity for most of the day, especially during typical working hours.
...
The weather comparison added another layer to that story.

Figure 2. Average pedestrian counts grouped by day type and rainfall condition. Wet weekdays show a clearer drop than wet weekends.
...
The city-wide pattern is clear, but Auckland CBD is not one uniform walking environment. Some sensors are much more weekday-heavy than others. A smaller number hold up better on weekends, even when overall weekend activity is still lower. That means the weekly shift is not evenly spread across the city centre.

Figure 3. The largest weekday–weekend differences across Auckland CBD sensors. Negative values indicate sensors that are busier on weekdays than weekends.
...
A ranked comparison of sensors shows this clearly. Some locations are strongly tied to weekday movement, while others show much smaller differences between weekdays and weekends. That makes sense intuitively: not every part of the CBD serves the same purpose, and not every street depends equally on workday routines.
...
Figure 4. Interactive sensor map showing whether locations are relatively more weekday-heavy or weekend-heavy. Most sensors are weekday-dominant, but the pattern is not evenly distributed across space.
...
To push the analysis a little further, I clustered sensors by their normalized weekday hourly profiles using KMeans. That means I did not group sensors by how large their counts were. I grouped them by the shape of their daily weekday pattern.

Figure 5. Average normalized weekday hourly profiles by sensor cluster. The clusters suggest that different parts of the CBD follow different weekday rhythms.
...
Figure 6. Sensor clusters mapped across Auckland CBD. The spatial pattern suggests that even weekday movement is made up of several different local rhythms.
...
The repository includes:
	...
	•	the data files,
	•	and the interactive map outputs used in the analysis.

Code walkthrough

Below is a short video walking through the Python workflow behind this analysis, including the data cleaning steps, custom functions, API integration, and figure generation.
